...
Public Class manage_published_customtab
    ...
    Private Sub BindGrid(ByVal pi As Integer)
        ...
        ds = obj.BindPublishedCustomTabs()

        If ds.Tables(0).Rows.Count > 0 Then
            grdCustomTab.DataSource = ds.Tables(0)
            grdCustomTab.DataBind()
            trTopPaging.Visible = True
            trBottomPaging.Visible = True
        Else
            grdCustomTab.DataSource = Nothing
            grdCustomTab.DataBind()
...
#Region "Create CSV"
    Sub Create_Csv()
        Try
            ...
            Dim ds As New DataSet
            ds = obj.DownLoadPublishedCustomTabs()

            Dim strPath As String = "PublishedCustomTab-Statistics-" & CStr(Day(Now)) & CStr(Month(Now)) & CStr(Year(Now)) & CStr(Hour(Now)) & CStr(Minute(Now)) & CStr(Second(Now)) & ".csv"
            Response.Expires = 0
            Response.Clear()
            ...
            For i = 0 To ds.Tables(0).Rows.Count - 1
                With ds.Tables(0).Rows(i)
                    Response.Write("""" & .Item("ct_Id").ToString() & """,")

                    Response.Write("""" & .Item("ct_Name").ToString() & """,")
                    '  Response.Write("""" & .Item("ct_Content").ToString() & """,")
                    Response.Write("""" & .Item("ct_Image").ToString() & """,")
                    Response.Write("""" & .Item("ct_UserId").ToString() & """,")
                    Response.Write("""" & .Item("ct_FBUserId").ToString() & """,")
                    Response.Write("""" & .Item("ct_CompanyId").ToString() & """,")
                    Response.Write("""" & .Item("ct_IndustryId").ToString() & """,")
                    Response.Write("""" & .Item("ct_CreatedDate").ToString() & """,")
                    Response.Write("""" & .Item("ct_UpdatedDate").ToString() & """,")
