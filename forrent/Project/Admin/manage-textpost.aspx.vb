...
Public Class manage_textpost
    ...
    Private Sub BindGrid(ByVal pi As Integer)
        Try
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                grdTextPost.DataSource = ds.Tables(0)
                grdTextPost.DataBind()
                trTopPaging.Visible = True
                trBottomPaging.Visible = True
            Else
                grdTextPost.DataSource = Nothing
                grdTextPost.DataBind()
...
#Region "Create CSV"
    Sub Create_Csv()
        Try
            ...
            Dim strPath As String = "TextPost-Statistics-" & CStr(Day(Now)) & CStr(Month(Now)) & CStr(Year(Now)) & CStr(Hour(Now)) & CStr(Minute(Now)) & CStr(Second(Now)) & ".csv"
            Response.Expires = 0
            Response.Clear()
