...
Public Class manage_quickpost
    ...
    Private Sub BindGrid(ByVal pi As Integer)
        Try
            ...
            obj.FromDate = ViewState("FromDate").ToString()
            obj.ToDate = ViewState("ToDate").ToString()
            obj.PageIndex = CInt(ViewState("CurrentPageIndex"))
            obj.SortBy = ViewState("SortBy").ToString()
            obj.Order = ViewState("Order").ToString()

            Dim ds As New DataSet
            ds = obj.BindQuickPost()

            If ds.Tables(0).Rows.Count > 0 Then
                grdQuickpost.DataSource = ds.Tables(0)
                grdQuickpost.DataBind()
                trTopPaging.Visible = True
                trBottomPaging.Visible = True
            Else
                grdQuickpost.DataSource = Nothing
                grdQuickpost.DataBind()
...
#End Region
    ...
    Private Sub btnDownload_ServerClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnDownload.ServerClick
        SearchQuickPostStatistics()
    End Sub

    Sub SearchQuickPostStatistics()
        ViewState("Company") = selCompany.Value
        ViewState("Industry") = selIndustry.Value
        ViewState("User") = selUser.Value
        If IsDate(txtFromDate.Value) Then
...
#Region "Create CSV"
    Sub Create_Csv()
        Try
            ...
            Dim ds As New DataSet
            ds = obj.DownLoadQuickPost()


            Dim strPath As String = "QuickPost-Statistics-" & CStr(Day(Now)) & CStr(Month(Now)) & CStr(Year(Now)) & CStr(Hour(Now)) & CStr(Minute(Now)) & CStr(Second(Now)) & ".csv"
            Response.Expires = 0
            Response.Clear()
            ...
            For i = 0 To ds.Tables(0).Rows.Count - 1
                With ds.Tables(0).Rows(i)
                    Response.Write("""" & .Item("sm_Id").ToString() & """,")

                    Response.Write("""" & .Item("sm_Message").ToString() & """,")
                    Response.Write("""" & .Item("sm_TSMAUserId").ToString() & """,")
                    Response.Write("""" & .Item("sm_CompanyId").ToString() & """,")
                    Response.Write("""" & .Item("sm_IndustryId").ToString() & """,")
                    Response.Write("""" & .Item("sm_Image").ToString() & """,")
                    Response.Write("""" & .Item("sm_Video").ToString() & """,")
                    Response.Write("""" & .Item("sm_VideoLink").ToString() & """,")
                    Response.Write("""" & .Item("sm_VideoImage").ToString() & """,")
                    Response.Write("""" & .Item("sm_Attachment").ToString() & """,")
                    Response.Write("""" & .Item("sm_ScheduleDate").ToString() & """,")
