...
Public Class manage_published_sweepstake
    ...
    Private Sub BindGrid(ByVal pi As Integer)
        ...
        ds = obj.BindPublishedSweepstakes()

        If ds.Tables(0).Rows.Count > 0 Then
            grdSweepstake.DataSource = ds.Tables(0)
            grdSweepstake.DataBind()
            trTopPaging.Visible = True
            trBottomPaging.Visible = True
        Else
            grdSweepstake.DataSource = Nothing
            grdSweepstake.DataBind()
...
#Region "Create CSV"
    Sub Create_Csv()
        Try
            ...
            Dim ds As New DataSet
            ds = obj.DownLoadPublishedSweepstakes()

            Dim strPath As String = "PublishedSweepstake-Statistics-" & CStr(Day(Now)) & CStr(Month(Now)) & CStr(Year(Now)) & CStr(Hour(Now)) & CStr(Minute(Now)) & CStr(Second(Now)) & ".csv"
            Response.Expires = 0
            Response.Clear()
            ...
            Response.Write(""" Facebook Page ID "",")
            Response.Write(""" Facebook Page Name "",")
            ...
            For i = 0 To ds.Tables(0).Rows.Count - 1
                With ds.Tables(0).Rows(i)
                    Response.Write("""" & .Item("ss_Id").ToString() & """,")

                    Response.Write("""" & .Item("ss_TSMAUserId").ToString() & """,")
                    Response.Write("""" & .Item("ss_FBUserId").ToString() & """,")
                    Response.Write("""" & .Item("ss_FBPageId").ToString() & """,")
                    Response.Write("""" & .Item("ss_FBPageName").ToString() & """,")
                    Response.Write("""" & .Item("ss_CompanyId").ToString() & """,")
                    Response.Write("""" & .Item("ss_IndustryId").ToString() & """,")
                    Response.Write("""" & .Item("ss_CreatedDate").ToString() & """,")
                    Response.Write("""" & .Item("ss_UpdatedDate").ToString() & """,")
