...
Public Class manage_imagepost
    ...
    Private Sub BindGrid(ByVal pi As Integer)
        Try
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                grdImagePost.DataSource = ds.Tables(0)
                grdImagePost.DataBind()
                trTopPaging.Visible = True
                trBottomPaging.Visible = True
            Else
                grdImagePost.DataSource = Nothing
                grdImagePost.DataBind()
...
#Region "Create CSV"
    Sub Create_Csv()
        Try
            ...
            Dim ds As New DataSet
            ds = obj.DownLoadTextPost()

            Dim strPath As String = "ImagePost-Statistics-" & CStr(Day(Now)) & CStr(Month(Now)) & CStr(Year(Now)) & CStr(Hour(Now)) & CStr(Minute(Now)) & CStr(Second(Now)) & ".csv"
            Response.Expires = 0
            Response.Clear()
