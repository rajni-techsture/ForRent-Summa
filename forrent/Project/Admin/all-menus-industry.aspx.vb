...
Public Class all_menus_industry
    ...
    Sub GenerateMenuTree()
        Try
            ...
            If IsNumeric(Request("Id")) Then
                ...
                Dim objmnu As New BALIndustry
                objmnu.IndustryId = intId
                DS = objmnu.ListMenus()
                ...
                If (DS.Tables(1).Rows.Count > 0) Then
                    hdnUserName.Value = DS.Tables(1).Rows(0).Item("i_Name").ToString()
                    hdnAdminUserName.Value = DS.Tables(1).Rows(0).Item("IndustryName").ToString()
                    objTree.AddRootNode("<b><font class=arial3b>" & DS.Tables(1).Rows(0).Item("IndustryName").ToString() & "</font></b>", "")
                End If
    ...
    Sub SaveMenuRights()
        Try
            Dim objSave As New BALIndustry
            ...
            Else
                strMenus = ""
            End If
            objSave.IndustryId = CInt(Request("Id"))
            objSave.CheckedMenu = strMenus
            objSave.UpdateAssignMenuRights()
           

            '-------------------- Write XML menu file start
            Dim objget As New BALIndustry
            objget.IndustryId = CInt(Request("Id"))
            DSAllMenus = objget.GetMenuRights()
            
            Dim DV As New DataView
            DV = DSAllMenus.Tables(0).DefaultView
            Dim strCurrentPath As String = Server.MapPath(ConfigurationManager.AppSettings("MenuPathIndustry"))
