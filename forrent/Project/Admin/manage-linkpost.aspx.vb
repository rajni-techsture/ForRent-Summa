...
Public Class manage_linkpost
    ...
    Private Sub BindGrid(ByVal pi As Integer)
        Try
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                grdLinkPost.DataSource = ds.Tables(0)
                grdLinkPost.DataBind()
                trTopPaging.Visible = True
                trBottomPaging.Visible = True
            Else
                grdLinkPost.DataSource = Nothing
                grdLinkPost.DataBind()
...
#Region "Create CSV"
    Sub Create_Csv()
        Try
            ...
            Dim ds As New DataSet
            ds = obj.DownLoadLinkPost()

            Dim strPath As String = "LinkPost-Statistics-" & CStr(Day(Now)) & CStr(Month(Now)) & CStr(Year(Now)) & CStr(Hour(Now)) & CStr(Minute(Now)) & CStr(Second(Now)) & ".csv"
            Response.Expires = 0
            Response.Clear()
