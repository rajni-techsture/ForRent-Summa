...
Public Class manage_autopost
    ...
    Private Sub BindGrid(ByVal pi As Integer)
        ...
        If ds.Tables(0).Rows.Count > 0 Then
            grdAutopost.DataSource = ds.Tables(0)
            grdAutopost.DataBind()
            trTopPaging.Visible = True
            trBottomPaging.Visible = True
        Else
            grdAutopost.DataSource = Nothing
            grdAutopost.DataBind()
...
#Region "Create CSV"
 ...
 Dim obj As New BALLoggingActivity
            ...
            Dim ds As New DataSet
            ds = obj.DownLoadAutoPost()

            Dim strPath As String = "AutoPostSent-Statistics-" & CStr(Day(Now)) & CStr(Month(Now)) & CStr(Year(Now)) & CStr(Hour(Now)) & CStr(Minute(Now)) & CStr(Second(Now)) & ".csv"
            Response.Expires = 0
            Response.Clear()
            ...
            For i = 0 To ds.Tables(0).Rows.Count - 1
                With ds.Tables(0).Rows(i)
                    Response.Write("""" & .Item("apm_Id").ToString() & """,")

                    Response.Write("""" & .Item("apm_Message").ToString() & """,")
                    Response.Write("""" & .Item("apm_TSMAUserId").ToString() & """,")
                    Response.Write("""" & .Item("apm_CompanyId").ToString() & """,")
                    Response.Write("""" & .Item("apm_IndustryId").ToString() & """,")
                    Response.Write("""" & .Item("apm_LibId").ToString() & """,")

                    Response.Write("""" & .Item("apm_Image").ToString() & """,")
                    Response.Write("""" & .Item("apm_Video").ToString() & """,")
                    Response.Write("""" & .Item("apm_VideoLink").ToString() & """,")
                    Response.Write("""" & .Item("apm_VideoImage").ToString() & """,")
                    Response.Write("""" & .Item("apm_Attachment").ToString() & """,")
                    Response.Write("""" & .Item("apm_ScheduleDate").ToString() & """,")
