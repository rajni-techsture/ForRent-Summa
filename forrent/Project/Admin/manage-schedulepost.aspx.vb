...
Public Class manage_schedulepost
    ...
    Private Sub BindGrid(ByVal pi As Integer)
        Try
            ...
            ds = obj.BindScheduledPost()

            If ds.Tables(0).Rows.Count > 0 Then
                grdSchduledPost.DataSource = ds.Tables(0)
                grdSchduledPost.DataBind()
                trTopPaging.Visible = True
                trBottomPaging.Visible = True
            Else
                grdSchduledPost.DataSource = Nothing
                grdSchduledPost.DataBind()
...
#Region "Create CSV"
    Sub Create_Csv()
        Try
            ...
            Dim ds As New DataSet
            ds = obj.DownLoadScheduledPost()

            Dim strPath As String = "ScheduledPost-Statistics-" & CStr(Day(Now)) & CStr(Month(Now)) & CStr(Year(Now)) & CStr(Hour(Now)) & CStr(Minute(Now)) & CStr(Second(Now)) & ".csv"
            Response.Expires = 0
            Response.Clear()
