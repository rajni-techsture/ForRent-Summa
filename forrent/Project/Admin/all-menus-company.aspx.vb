...
Public Class all_menus_company
    ...
    Sub GenerateMenuTree()
        Try
            ...
            If IsNumeric(Request("Id")) Then
                ...
                Dim objmnu As New BALCompany
                objmnu.CompanyId = intId
                DS = objmnu.ListMenus()
                ...
                If (DS.Tables(1).Rows.Count > 0) Then
                    hdnUserName.Value = DS.Tables(1).Rows(0).Item("c_Name").ToString()
                    hdnAdminUserName.Value = DS.Tables(1).Rows(0).Item("CompanyName").ToString()
                    objTree.AddRootNode("<b><font class=arial3b>" & DS.Tables(1).Rows(0).Item("CompanyName").ToString() & "</font></b>", "")
                End If
    ...
    Sub SaveMenuRights()
        Try
            ...
            Else
                strMenus = ""
            End If
            objSave.CompanyId = CInt(Request("Id"))
            objSave.CheckedMenu = strMenus
            objSave.UpdateAssignMenuRights()
           

            '-------------------- Write XML menu file start
            Dim objget As New BALCompany
            objget.CompanyId = CInt(Request("Id"))
            DSAllMenus = objget.GetMenuRights()
           
            Dim DV As New DataView
            DV = DSAllMenus.Tables(0).DefaultView
            Dim strCurrentPath As String = Server.MapPath(ConfigurationManager.AppSettings("MenuPathCompany"))
