...
Public Class manage_videopost
    ...
    Private Sub BindGrid(ByVal pi As Integer)
        Try
            ...
            ds = obj.BindVideoPost()

            If ds.Tables(0).Rows.Count > 0 Then
                grdVideoPost.DataSource = ds.Tables(0)
                grdVideoPost.DataBind()
                trTopPaging.Visible = True
                trBottomPaging.Visible = True
            Else
                grdVideoPost.DataSource = Nothing
                grdVideoPost.DataBind()
...
#End Region
    ...
    Private Sub btnDownload_ServerClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnDownload.ServerClick
        SearchScheduledPostStatistics()
    End Sub

    Sub SearchScheduledPostStatistics()
        ViewState("Company") = selCompany.Value
        ViewState("Industry") = selIndustry.Value
        ViewState("User") = selUser.Value
        If IsDate(txtFromDate.Value) Then
...
#Region "Create CSV"
    Sub Create_Csv()
        Try
            ...
            Dim ds As New DataSet
            ds = obj.DownLoadVideoPost()

            Dim strPath As String = "VideoPost-Statistics-" & CStr(Day(Now)) & CStr(Month(Now)) & CStr(Year(Now)) & CStr(Hour(Now)) & CStr(Minute(Now)) & CStr(Second(Now)) & ".csv"
            Response.Expires = 0
            Response.Clear()
            ...
            For i = 0 To ds.Tables(0).Rows.Count - 1
                With ds.Tables(0).Rows(i)
                    ...
                    Response.Write("""" & .Item("sm_PostType").ToString() & """,")
