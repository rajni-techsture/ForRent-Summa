    ...
    Public Function BindCoverPhoto()
        Try
            If Session("FacebookAccessToken") <> Nothing AndAlso Session("FacebookAccessToken") <> "" Then
                ...
                ds = objBAL.GetCoverPhotoMasterByCompOrIndustry
                If ds.Tables(0).Rows.Count > 0 Then
                    rptCoverPhoto.DataSource = ds.Tables(0)
                    rptCoverPhoto.DataBind()
                Else
                    lblMessage.Text = "No Cover Photo Found"
                    pnlCarousel.Visible = "False"
                    rptCoverPhoto.DataSource = Nothing
                    rptCoverPhoto.DataBind()
                End If
            Else
                Response.Redirect(System.Configuration.ConfigurationManager.AppSettings("AppPath"))
        ...
        End Try
    End Function
    Public Function BindCoverPhotoAll()
        Try
            If Session("FacebookAccessToken") <> Nothing AndAlso Session("FacebookAccessToken") <> "" Then
                Dim ds As New DataSet
                Dim objBAL As New BALCoverPhoto
        ...
        End Try
    End Function
    Public Function BindCoverPhotoByPageIndex()
        Try
            If Session("FacebookAccessToken") <> Nothing AndAlso Session("FacebookAccessToken") <> "" Then
                Dim ds As New DataSet
                Dim objBAL As New BALCoverPhoto
